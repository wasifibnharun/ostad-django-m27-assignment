...
export default function DashboardPage() {
  ...
  const { data: aliveData, isLoading: loadingAlive } = useQuery({
    ...
    queryFn: () =>
      fetch(`${BASE_URL}/character?status=alive`).then((res) => res.json()),
  });

  const { data: deadData, isLoading: loadingDead } = useQuery({
    queryKey: ["character", "meta", "dead"],
    queryFn: () =>
      fetch(`${BASE_URL}/character?status=dead`).then((res) => res.json()),
  });
      ...
      >
        ...
        {loadingAlive ? (
          ...
          <StatCard
            ...
            subtext={
              <span>
                <strong style={{ color: "#047857" }}>{alivePct}%</strong> of all
                characters
              </span>
            }
            icon="💚"
        ...
        {loadingDead ? (
          ...
          <StatCard
            ...
            subtext={
              <span>
                <strong style={{ color: "#B91C1C" }}>{deadPct}%</strong> of all
                characters
              </span>
            }
            icon="🕯️"
